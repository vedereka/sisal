# ************************************************************************ #
#                                                                          #
#              EXAMPLE CODES FOR LINKING MySQL to R                        #
#                                                                          #
...
# IMPORT LIBRARY -------------------------------------------------------####
# Technically RMySQL is only required
library(RMySQL)
# Other packages here are just for plotting
library(ggplot2)
library(ggmap)
library(maps)
# packages used for making analogous figures to figures 1 and 2 in Atsawawaranunt et al., 2018
library(plyr)
...
dbListTables(mydb)
# List fields from site table 
dbListFields(mydb, 'site')
...
dating_tb <- dbGetQuery(mydb, 'SELECT * FROM dating WHERE entity_id = 1;') 
# sample depth/isotope table (with hiatuses)
sample_tb <- dbGetQuery(mydb, 'SELECT * FROM sample LEFT JOIN hiatus USING (sample_id) LEFT JOIN d13C USING (sample_id) LEFT JOIN d18O USING (sample_id) WHERE entity_id = 1')
...
dbExecute(mydb, 'SET SESSION group_concat_max_len = 100000;')
tot_ref <- dbGetQuery(mydb, 'SELECT entity.entity_id, site.site_name as "Site name", 
...
# example of how Figure 1 was made -------------------------------------####
db_version <- 'version 2'
tot_site <- dbGetQuery(mydb, "SELECT site.*, count(*) as entity_count FROM site LEFT JOIN entity USING (site_id) WHERE entity_status = 'current' GROUP BY (site_id);")
tot_d18Osite <- dbGetQuery(mydb, "SELECT site_id, site_name, latitude, longitude, elevation, entity_id, entity_name FROM site LEFT JOIN entity USING (site_id) JOIN sample USING(entity_id) JOIN d18O USING(sample_id) GROUP BY (entity_id) HAVING COUNT(*) > 0;")
tot_d13Csite <- dbGetQuery(mydb, "SELECT site_id, site_name, latitude, longitude, elevation, entity_id, entity_name FROM site LEFT JOIN entity USING (site_id) JOIN sample USING(entity_id) JOIN d13C USING(sample_id) GROUP BY (entity_id) HAVING COUNT(*) > 0;")
...
#
# tot_site_entity <- dbGetQuery(mydb, "SELECT * FROM site JOIN entity USING (site_id);")
...
# example of how Figure 2 was made -------------------------------------####
...
full_sample_tb <- dbGetQuery(mydb, "SELECT site.*, entity.entity_name, sample.*, original_chronology.* FROM site JOIN entity USING(site_id) JOIN sample USING(entity_id) JOIN original_chronology USING(sample_id);")
